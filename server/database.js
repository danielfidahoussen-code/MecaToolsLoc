...
// Seed admin
if (users.count() === 0) {
  users.insert({ email: 'admin@prestolocation.re', password: bcrypt.hashSync(process.env.ADMIN_PASSWORD || 'admin123', 10), role: 'admin' });
}
...
if (faqs.count() === 0) {
  [
    ...
    { question: 'Comment contacter le service client ?',        answer: 'Vous pouvez nous joindre par téléphone au 06 93 83 96 54 ou par email à contact@prestolocation.re. Nous répondons sous 24h.', category: 'contact', sort_order: 8 },
...
faqs.all().forEach(f => {
  if (f.answer && f.answer.includes('Locationautopresto@gmail.com')) {
    faqs.update(f.id, { answer: f.answer.replaceAll('Locationautopresto@gmail.com', 'contact@prestolocation.re') });
  }
});
